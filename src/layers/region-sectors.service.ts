...
export class RegionSectorsService implements LayerService {
  ...
  private isSectorBase(type: MapStructure): boolean {
    return (
      type === MapStructure.TownBase1 ||
      type === MapStructure.TownBase2 ||
      type === MapStructure.TownBase3 ||
      type === MapStructure.RelicBase1 ||
      type === MapStructure.RelicBase2 ||
      type === MapStructure.RelicBase3
    );
  }

  private getTeamColour(teamId: TeamId): string {
    const theme = this.themeService.vibrancy();
    switch (teamId) {
      case 'COLONIALS':
    ...
    return this.warApiService.getAllMapDynamicData(shard).pipe(
      ...
      map((mapData) => {
        ...
        for (const [name, data] of Object.entries(mapData)) {
          const points = data.mapItems
            .filter((item) => this.isSectorBase(item.iconType))
            .map((mapItem) => {
              const [x, y] = this.hexCoordinateService.normaliseCoordinates(
                name as HexName,
                [mapItem.x, mapItem.y]
              );
              return {
                point: new Point([x, y]),
                colour: this.getTeamColour(mapItem.teamId),
              };
            });

          let [sX, sY] = this.hexCoordinateService.getCoordinates(
            name as HexName
          );
          const vertices = HexOverlayService.drawHex(
            sX,
            sY,
            this.hexCoordinateService.hexSize
          );
          features.push(
            ...this.generateRegionPolygons(
              points,
              new Polygon([[...vertices, vertices[0]]])
            )
          );
        }
  ...
  private generateRegionPolygons(
    points: { point: Point; colour: string }[],
    hexPolygon: Polygon
  ): Feature[] {
    ...
    const voronoiPolygons = voronoi(featureCollection(featurePoints), {
      bbox: boundingBox,
    });
    ...
    return voronoiPolygons.features
      ...
      .map((voronoiFeature) => {
        const voronoiPoly =
          voronoiFeature.geometry.type === 'Polygon' ||
        ...
        const intersection = intersect(
          featureCollection([featureVoronoiPoly, featureTurfHex])
        );
        if (intersection) {
